...
def login():
    try:
        ...
        # Generate JWT token - FIXED: use timedelta directly
        token_payload = {
            'email': user['email'],
            'role': user['role'],
            'exp': datetime.utcnow() + timedelta(hours=24)  # FIXED LINE
        }
    ...
    def generate_pdf_report(self, scan_data, vulnerabilities, report_id):
        ...
        try:
            ...
            overview_data = [
                ...
                ['Scan Owner', scan_data.get('user_email', 'Unknown')]
            ]
        ...
        try:
            report = {
                ...
                "scan_summary": {
                    ...
                    "scan_owner": scan_data.get('user_email', 'Unknown')
                },
    ...
    def generate_csv_report(self, scan_data, vulnerabilities, report_id):
        ...
        try:
            ...
            with open(filename, 'w', newline='') as csvfile:
                ...
                writer.writerow(['Scan Owner:', scan_data.get('user_email', 'Unknown')])
    ...
    def generate_executive_summary(self, scan_data, vulnerabilities, report_id):
        ...
        try:
            ...
            with open(filename, 'w') as f:
                ...
                f.write(f"Scan Owner: {scan_data.get('user_email', 'Unknown')}\n\n")
...
class RealTimeSecurityScanner:
    ...
    def run_scan(self, scan_id, target, scan_type, user_email):
        try:
            ...
            logger.info(f"Scan {scan_id} completed with {len(vulnerabilities)} findings for user {user_email}")
...
def start_scan(current_user):
    ...
    try:
        ...
        thread = threading.Thread(
            ...
            args=(scan_id, target, scan_type, current_user['email'])
        )
        thread.daemon = True
        thread.start()
...
def download_report(current_user, report_id):
    try:
        ...
        if report_info.get('user_email') != current_user['email'] and current_user['role'] != 'admin':
            return jsonify({'error': 'Access denied'}), 403
...
def list_reports(current_user):
    if current_user['role'] == 'admin':
        ...
        reports = [r for r in reports_db.values() if r.get('user_email') == current_user['email']]
...
def get_all_scans(current_user):
    ...
    if current_user['role'] == 'admin':
        ...
        scans = [s for s in scans_db.values() if s.get('user_email') == current_user['email']]
...
def get_assets(current_user):
    ...
    if current_user['role'] == 'admin':
        user_scans = list(scans_db.values())
    else:
    ...
    for scan in user_scans:
        ...
        assets_map[target]['scan_count'] += 1
        if scan['start_time'] > assets_map[target]['last_scanned']:
            assets_map[target]['last_scanned'] = scan['start_time']
    
    assets = list(assets_map.values())
    assets.sort(key=lambda x: x['last_scanned'], reverse=True)
...
def get_stats(current_user):
    if current_user['role'] == 'admin':
        user_scans = list(scans_db.values())
        user_vulnerabilities = vulnerabilities_db
    else:
        user_scans = [s for s in scans_db.values() if s.get('user_email') == current_user['email']]
        user_vulnerabilities = {scan_id: vulns for scan_id, vulns in vulnerabilities_db.items() 
                              if scans_db.get(scan_id, {}).get('user_email') == current_user['email']}
    
    total_scans = len(user_scans)
    completed_scans = len([s for s in user_scans if s['status'] == 'completed'])
    running_scans = len([s for s in user_scans if s['status'] == 'running'])
    failed_scans = len([s for s in user_scans if s['status'] in ['failed', 'terminated']])
    
    vuln_stats = {'critical': 0, 'high': 0, 'medium': 0, 'low': 0, 'info': 0}
    for scan_id, vulns in user_vulnerabilities.items():
        for vuln in vulns:
    ...
    return jsonify({
        ...
        'total_reports': len([r for r in reports_db.values() if r.get('user_email') == current_user['email']]),
